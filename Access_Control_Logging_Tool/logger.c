...
fopen(const char *path, const char *mode) 
{
	...
	//i need to take all the contents of the file
	if(original_fopen_ret != NULL)
	{

		// in the original_fopen_ret i have the value that comes up when the original fopen is coming so i am 
		// using it to determine the size of the file.
		...
		fread(filecontents,1,file_size,original_fopen_ret);


		// continue the md5 process 
		MD5_Update(&ctx,filecontents,file_size);
